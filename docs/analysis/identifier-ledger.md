...
| `ut` / chunk | 6 | Exact addon sound-root literal is selected at 5112, optionally extended with `q_`, and prefixes every bundled sound filename. | `soundRootPath` | resolved | High: initializer and complete sound-manager consumers. | 01–11 | Core/Audio | Preserve addon-owned path casing. |
...
| `ft` / chunk | 8 | Seven colored gem-name strings indexed by maximum `stats.gemMatch` entry for favorite-color display (7026–7035). | `coloredGemDisplayNames` | resolved | High. | 01–15 | UI/Stats | Tie logic is asymmetric. |
| `he` / chunk | 169 | Nine RGB triples color lightning, shards, and floating text (2614, 2636, 2714–2717); entries 1–7 match gem colors and entries 8–9 are white special/fallback colors before match scanner `he` shadows the binding. | `gemEffectColors` | resolved | High: all table consumers and fallback behavior are visible. | 01–06 | Core/Constants, UI/Animations | None. |
| `U` / chunk | 180 | Numeric keys 1–7 map to lowercase color suffixes used in `gem_`, `shine_`, and `highlight_` asset paths and gem construction (2050–2065, 2486–2488), before tab handler `U` shadows it. | `gemColorTextureNames` | resolved | High: complete table and asset-name consumers. | 01–07 | Core/Constants, UI/GemPool | None. |
...
| `J` / chunk | 183 | Receives copies of `F`'s 25 rectangles and supplies explosion texture frames from initial frame 1 through terminal frame 16 (204–208, 2558, 4730–4744). | `explosionAtlasRects` | resolved | High: copy construction and exclusive explosion consumers. | 01–10 | UI/Animations | Preserve copied table identity. |
...
| `ie` / chunk | 185 | Sixteen 4×4 UV rectangles; shard creation starts at frame 1 and animator advances `gemshards` through the first 12 frames (211–217, 2635, 4698–4729). | `shardAtlasRects` | resolved | High: table geometry, texture owner, and frame consumer are complete. | 01–10 | UI/Animations | Four generated cells remain unused. |
...
| `Ye` / chunk | 228 | Constant `24`, halved into `pt`, and passed as shard image width at 2628 before mouse-enter function `Ye` shadows it. | `shardWidth` | resolved | High: direct constructor argument and half-width arithmetic. | 01–06 | UI/Animations | None. |
| `Ze` / chunk | 229 | Constant `24`, halved into `ct`, and passed as shard image height at 2628 before command function `Ze` shadows it. | `shardHeight` | resolved | High: direct constructor argument and half-height arithmetic. | 01–08 | UI/Animations | None. |
| `b` / chunk | 230 | Constant `50`; halved into `ce`, drives horizontal match offsets, and spaces final gem-frame X positions (283, 1013, 8375). | `gemWidth` | resolved | High: direct X-coordinate geometry. | 01–17 | UI/GemPool | None. |
| `p` / chunk | 231 | Constant `50`; copied/halved, drives vertical match offsets, and spaces final gem-frame Y positions (272, 274, 284, 1014, 8375). | `gemHeight` | resolved | High: direct Y-coordinate geometry. | 01–17 | UI/GemPool | None. |
| `lt` / chunk | 232 | Constant `90` sizes big-star texture/highlight height at 2526/2529 before lightning factory `lt` shadows it. | `bigStarTextureHeight` | resolved | High: paired direct frame geometry. | 01–06 | UI/Animations | None. |
| `Ue` / chunk | 233 | Constant `90` sizes big-star texture/highlight width at 2527/2530 before level-advance function `Ue` shadows it. | `bigStarTextureWidth` | resolved | High: paired direct frame geometry. | 01–06 | UI/Animations | None. |
| `Qe` / chunk | 234 | Constant `150` sizes explosion texture height at 2551; its dead half derivation is shadowed before clear helper `Qe` replaces the binding. | `explosionTextureHeight` | resolved | High: direct effect-factory geometry. | 01–06 | UI/Animations | None. |
| `qe` / chunk | 235 | Constant `150` sizes explosion texture width at 2552; its dead half derivation is shadowed before shard factory `qe` replaces the binding. | `explosionTextureWidth` | resolved | High: direct effect-factory geometry. | 01–06 | UI/Animations | None. |
| `s` / chunk | 236 | Constant `400`; derives main-window width and directly sizes the final game surface/tiles (`s / 4`) and board (`s + 14`) (238, 8041, 8054, 8064, 8068). | `boardWidth` | resolved | High: final construction confirms 8 × 50-pixel board geometry. | 01–17 | Engine/Grid, UI/GemPool | None. |
| `w` / chunk | 237 | Constant `400`; derives main-window height and directly sizes the final game surface/tiles (`w / 4`) and board (`w + 12`) (239, 8039, 8042, 8055, 8065, 8069). | `boardHeight` | resolved | High: final construction confirms paired board geometry. | 01–17 | Engine/Grid, UI/GemPool | None. |
...
| `Je` / chunk | 242 | Constant `10` bounds per-clear shard creation at 2663; `gt = 5` divides their initial direction signs before floating-text factory `Je` shadows it. | `shardsPerClear` | resolved | High: loop bound and paired half-count behavior. | 01–06 | UI/Animations | None. |
...
| `S` / chunk | 244 | Effect constant `-1`; assigned when hint animation is reset/disabled (527, 7773) and excluded from interactive/animator gem processing, then shadowed by event dispatcher at 7874. | `FX_INACTIVE` | resolved | High. | 01–16 | UI/Animator, UI/Options | Preserve numeric effect value. |
...
| `Lt` / chunk | 246 | Constant `20` wraps normal gem-atlas animation frames at 4329–4340. | `normalGemFrameCount` | resolved | High: exclusive animation-loop bound. | 01–09 | UI/Animations | None. |
| `ye` / chunk | 247 | Effect enum `3` assigned to big-star objects and dispatched by the rotating/glow branch at 4648–4697. | `FX_BIG_STAR` | resolved | High: assignment, queue lifecycle, and complete branch. | 01–10 | UI/Animations | Preserve numeric value. |
| `ve` / chunk | 248 | Constant `360` wraps both opposing big-star rotation angles at 4651–4655. | `FULL_ROTATION_DEGREES` | resolved | High: exact angular consumers. | 01–10 | UI/Animations | Preserve legacy wrap expression. |
| `Oe` / chunk | 249 | Effect enum `4` assigned by explosion factory and dispatched/pooled by the explosion branch at 4730–4745. | `FX_EXPLOSION` | resolved | High: complete object lifecycle. | 01–10 | UI/Animations | Preserve numeric value. |
| `bt` / chunk | 250 | Constant `16` terminates explosion atlas playback at 4732. | `explosionFrameCount` | resolved | High: exclusive frame bound. | 01–10 | UI/Animations | None. |
| `He` / chunk | 251 | Effect enum `5` assigned to shard objects and dispatched/pooled by the shard physics branch at 4698–4729. | `FX_SHARD` | resolved | High: complete object lifecycle. | 01–10 | UI/Animations | Preserve numeric value. |
| `Xe` / chunk | 252 | Constant `12` bounds randomized shard start frames and shard atlas playback (2641, 4700). | `shardFrameCount` | resolved | High: creation and update bounds agree. | 01–10 | UI/Animations | Generated atlas cells 13–16 are unused. |
...
| `se` / chunk | 254 | Constant `4` is the frame count used to shrink ordinary big-star cells during clear resolution (4511–4520). | `bigStarShrinkFrameCount` | resolved | High: complete size derivation and terminal branch. | 01–10 | UI/Animations | Distinct from effect enum `Oe`. |
| `je` / chunk | 255 | Effect enum `7` starts mouse-over shine propagation; it advances `FX_SHINE_ALPHA`, chooses adjacent cells, and spawns `ke` highlights (2760–2765, 4341–4386). | `FX_HOVER_SHINE` | resolved | High: assignment and complete propagation branch. | 01–09 | UI/Animations, UI/GemPool | Preserve numeric value. |
| `it` / chunk | 256 | `#FX_SHINE_ALPHA` (=6); initializes hover shine at its final alpha frame and bounds each propagation cycle (2761, 4343). | `hoverShineFrameCount` | resolved | High: exact initializer and loop bound. | 01–09 | UI/Animations | None. |
| `ke` / chunk | 257 | Effect enum `8` marks an adjacent propagated shine and is dispatched until `Re` completes or reverses (3048–3050, 4350–4401). | `FX_ADJACENT_SHINE` | resolved | High: complete assignment/reversal/update lifecycle. | 01–09 | UI/Animations, UI/GemPool | Preserve numeric value. |
| `Re` / chunk | 258 | `#FX_SHINE_ALPHA` (=6); reverses adjacent hover frames on leave and terminates the `ke` update branch (3049–3050, 4389). | `adjacentShineFrameCount` | resolved | High: distinct reversal/termination role. | 01–09 | UI/Animations | Same numeric value as `it`, separate semantic role. |
...
| `mt` / chunk | 260 | Constant `40` wraps the hyper-gem texture atlas at 4620–4627. | `hyperGemFrameCount` | resolved | High: exclusive atlas-frame bound. | 01–10 | UI/Animations | None. |
...
| `be` / chunk | 262 | Constant `10` starts floating-text fade and, at double that value, terminates/returns text to its pool (4769–4776). | `floatingTextFadeFrame` | resolved | High: complete two-stage lifetime. | 01–10 | UI/Animations | Distinct from effect enum `te`. |
...
| `Ve` / chunk | 264 | Effect enum `12` assigned to the hint arrow and dispatched by its delayed vertical-bounce branch (2576–2582, 4787–4805). | `FX_HINT_BOUNCE` | resolved | High: complete assignment and update behavior. | 01–10 | UI/Animations, UI/HUD | Preserve numeric value. |
| `yt` / chunk | 265 | Constant `25` delays hint-arrow visibility/bounce until accumulated effect time exceeds it (4789–4804). | `hintDisplayDelay` | resolved | High: sole threshold consumer. | 01–10 | UI/Animations, UI/HUD | Units follow animator elapsed accumulation. |
| `De` / chunk | 266 | Effect enum `13` assigned to lightwave objects and dispatched through propagation/fade/pooling (2598–2601, 4305–4318). | `FX_LIGHTWAVE` | resolved | High: complete lifecycle. | 01–09 | UI/Animations | Preserve numeric value. |
| `We` / chunk | 267 | Effect enum `14` assigned to lightning textures and dispatched through alternating highlight/frame/pooling behavior (2615, 4746–4765). | `FX_LIGHTNING` | resolved | High: complete lifecycle. | 01–10 | UI/Animations | Preserve numeric value. |
| `Rt` / chunk | 268 | Constant `15` terminates lightning effect playback at 4748. | `lightningFrameCount` | resolved | High: exclusive frame bound. | 01–10 | UI/Animations | None. |
...
| `_e` / chunk | 271 | Movement enum `50` marks reciprocal gem swaps and selects the invalid-reversal/valid-completion branch (3119, 3136, 4807–4882). | `MOVE_SWAP` | resolved | High: assignment sites and complete dispatcher branch. | 01–10 | Engine/Swaps, UI/Animations | Preserve numeric value. |
| `ue` / chunk | 272 | Copies gem height `50`; clamps completed swap X/Y offsets to exactly one cell at 4811–4820. | `swapCellDistance` | resolved | High: complete movement geometry. | 01–10 | Engine/Swaps, UI/Animations | Same value applies to both axes on square grid. |
...
| `le` / chunk | 275 | Movement enum `51` marks falling/refill gems, including big-star propagation; animator integrates/clamps them in drop and generic falling branches (3921, 4480, 4494, 4524–4617). | `MOVE_FALLING` | resolved | High: assignments and complete physics dispatch. | 01–10 | Engine/Cascade, UI/Animations | Preserve numeric value. |
...
| `at` / chunk | 277 | Movement enum `53` is assigned to all gems by board-wipe helper `Ke` and selects random-velocity/free-flight cleanup at 4883–4903. | `MOVE_BOARD_WIPE` | resolved | High: assignment and complete dispatcher lifecycle. | 01–10 | UI/Animations, Engine/Scoring | Preserve numeric value. |
| `t` / chunk (line 278) | 278 | Constant `16`; shadowed at 287. | unknown enum sixteen | shadowed | Low. | 01–01 | Engine | Locate any pre-287 read. |
| `Te` / chunk | 279 | Constant velocity `20`; shards and board-wipe gems terminate at it, while half `oe = 10` begins fade (4718–4721, 4889–4892). | `effectTerminalVelocity` | resolved | High: paired physics thresholds. | 01–10 | UI/Animations | None. |
| `et` / chunk | 280 | Constant `150` becomes per-update swap offset step `et × elapsed` at 4247; a separate dead derivation at 4250 has no effect. | `swapSpeed` | resolved | High: direct time-scaled movement consumer. | 01–09 | Engine/Swaps, UI/Animations | Preserve elapsed scaling. |
| `wt` / chunk | 281 | Constant `60` becomes per-update falling acceleration step `wt × elapsed` at 4248 and is applied to drop physics. | `fallAcceleration` | resolved | High: direct time-scaled physics consumer. | 01–09 | Engine/Cascade, UI/Animations | Preserve elapsed scaling. |
| `Gt` / chunk | 282 | Constant `10` becomes per-update floating-text frame/position step `Gt × elapsed` at 4249. | `floatingTextSpeed` | resolved | High: complete downstream movement/fade consumers. | 01–10 | UI/Animations | Preserve elapsed scaling. |
...
| `pt` / chunk | 285 | Half shard width `12`; centers randomized shard X creation around a gem at 2683. | `halfShardWidth` | resolved | High: direct coordinate derivation. | 01–06 | UI/Animations | None. |
| `ct` / chunk | 286 | Half shard height `12`; centers randomized shard Y creation around a gem at 2683. | `halfShardHeight` | resolved | High: direct coordinate derivation. | 01–06 | UI/Animations | None. |
...
| `oe` / chunk | 289 | Half terminal velocity `10`; begins linear shard/board-wipe fade before `Te = 20` terminates motion (4720–4721, 4891–4892). | `effectFadeVelocity` | resolved | High: complete fade interval. | 01–10 | UI/Animations | None. |
| `gt` / chunk | 290 | Half of ten shards (`5`); splits initial shard direction sign in the creation loop at 2664–2667. | `halfShardCount` | resolved | High: exact loop partition. | 01–06 | UI/Animations | None. |
...
| `Pe` / chunk | 294 | Animator state `1` processes marked clears, scoring, special creation/triggers, and is entered after valid swap or detected match (4150, 4403–4522, 4867–4875). | `ANIM_RESOLVE_CLEARS` | resolved | High: state entries and complete dispatcher branch. | 01–10 | Engine/StateMachine | Preserve numeric value. |
| `Ct` / chunk | 295 | Animator state `2` invokes jewel falling/collapse; local state is advanced to 3 before transition logic advances persistent state to refill `4` (4298–4301). | `ANIM_COLLAPSE` | resolved | High: exclusive state comparison and transition. | 01–09 | Engine/StateMachine, Engine/Cascade | Preserve transient local state-3 behavior. |
| `t` / chunk (line 296) | 296 | Constant `3`; shadowed at 306. | unknown enum three | shadowed | Low. | 01–01 | Engine | Locate any pre-306 read. |
| `pe` / chunk | 297 | Animator state `4` creates/refills falling jewels; entered for new games and post-cascade refill and dispatched at 4295/4523. | `ANIM_REFILL` | resolved | High: state entries, factory call, and physics branch. | 01–10 | Engine/StateMachine, Engine/Cascade | Preserve numeric value. |
| `ot` / chunk | 298 | Animator state `5` is the post-refill match-check point; new-game startup can collapse it to idle, otherwise it runs match detection (4144–4150). | `ANIM_CHECK_MATCHES` | resolved | High: complete transition conditions. | 01–09 | Engine/StateMachine, Engine/Matches | Preserve numeric value. |
| `Fe` / chunk | 299 | Animator state `6` runs board-wipe/level-transition motion; end-game and level-up enter it, and completion above it selects summary or refill (2453, 4147–4179). | `ANIM_BOARD_TRANSITION` | resolved | High: all entries and exit behavior. | 01–09 | Engine/StateMachine, UI/Animations | Preserve numeric value. |
| `nt` / chunk | 300 | Match-axis enum `1`; passed for vertical runs/cross arms and causes mark helper `Le` to set `markY` (2889, 2981). | `MATCH_AXIS_VERTICAL` | resolved | High: both call orientations and helper branch. | 01–07 | Engine/Matches | Preserve numeric value. |
| `Ne` / chunk | 301 | Match-axis enum `2`; `Le` maps it to `markX`, and horizontal runs/cross arms pass it (2286–2289, 2893, 2977). | `MATCH_AXIS_HORIZONTAL` | resolved | High: helper identity and both call orientations. | 01–07 | Engine/Matches | Preserve numeric value. |
...
| `c` / chunk | 309 | Constant `1`; keys `Ce`; controls classic score/bar/results/pause behavior and final New Game/splash classic launches (467, 500, 536, 611, 881, 1278–1352, 8245–8248, 8344–8348). | `GAME_MODE_CLASSIC` | resolved | High: complete-file explicit classic behavior. | 01–17 | Core/Constants | None. |
...
| `re` / chunk (numeric overwrite) | 321 | Overwrites temporary `tonumber` alias with `40`; animator glow-frame oscillator increments, clamps, and normalizes against it (4263–4272). | `glowCycleMaximum` | resolved | High: complete oscillator arithmetic. | 01–09 | UI/Animations | Split from captured conversion alias `D`. |
| `z` / chunk (numeric overwrite) | 322 | Overwrites temporary `type` alias with `7`; caps weighted hyper/big-star neighborhood glow and participates in normalized alpha scale (4272, 4635–4637, 4685–4687). | `maximumGlowLevel` | resolved | High: complete cap and scaling consumers. | 01–10 | UI/Animations | Split from captured type alias `R`. |
| `o` / chunk | 323 | Eight row tables initialized/exported as `debugArray`, traversed as gem frames, and populated with all 64 final one-based gem objects (324–326, 442, 1275–1293, 8382–8384). | `gemGrid` | resolved | High: explicit 8×8 construction and traversal. | 01–17 | Engine/Grid, UI/GemPool | Debug export naming is incidental. |
| `e` / debug-array loop | 324 | Loop 1–8; indexes `o` for writes (325). | `index` | resolved | High: direct loop role. | 01–01 | Core/Init | None. |
| `v` / chunk | 327 | Key for authenticated personal-best payloads; final bootstrap assigns `x(1378301, 4)`, which deterministically encodes to literal `data` (887, 900, 3490–3491, 3601–3613, 8004). | `statDataKey` | resolved | High: complete consumers plus exact base-70 derivation `d-a-t-a`. | 01–17 | Core/SavedVariables | Preserve `data` in profile wire format. |
...
| `we` / chunk | 327 | True after level-up or restoring a game above level 1, nil for a fresh level-1 game; during level spawn it preserves removed hyper/big-star counts for re-placement (529, 2240–2242, 3890–3906, 3961–4026). | `preserveSpecialsAcrossLevelSpawn` | resolved | High: all writes, guard, removal counts, and restoration consumers are complete. | 01–09 | Engine/Scoring, Engine/Cascade | Persistent truth across later levels is intentional legacy behavior. |
| `r` / chunk | 333 | Six four-number scan tuples drive initial-board anti-match checks; the first four also provide orthogonal legal-swap offsets (2004–2047, 2302–2346). | `neighborScanPatterns` | resolved | High: all tuple fields and both consumers are complete. | 01–05 | Engine/Grid, Engine/Matches | Entries 5–6 encode centered horizontal/vertical three-cell scans. |
...
| `st` / chunk function | 455 | Queues an object only when `.animated` is false, sets the guard, and is attached as animator method `Add` at 4952. | `addAnimationOnce` | resolved | High: complete body and final method attachment. | 01–10 | UI/Animations | None. |
| `t` / `st` parameter | 455 | Animator receiver whose `animationStack` receives the object; final attachment proves concrete owner type. | `animator` | resolved | High. | 01–10 | UI/Animations | None. |
| `e` / `st` parameter | 455 | Effect/frame object whose `.animated` guard prevents duplicate stack insertion. | `animationObject` | resolved | High: complete lifecycle across animator removal sites. | 01–10 | UI/Animations | None. |
| `ht` / chunk function | 462 | Writes score/game state, checks thresholds, emits level/multiplier UI/audio, marks level-up, then updates bar; attached as `levelBar.SetScore` at 8198. | `setScore` | resolved | High: complete body and final method attachment. | 01–17 | Engine/Scoring, UI/HUD | None. |
...
| `Tt` / chunk function | 535 | Complete classic score/non-classic timer bar rendering body; attached as `levelBar.UpdateBar` at 8202. | `updateLevelBar` | resolved | High: complete body, UI effects, and attachment. | 02–17 | UI/HUD | None. |
...
| `x` / chunk function | 682 | Converts numbers to fixed-width base-70 text; score/network/save authentication consumes it, and final `x(1378301, 4)` proves stat key `data` (683–711, 818–853, 887–911, 1933, 8004). | `encodeBase70` | resolved | High: complete body, consumers, and exact final constant derivation. | 02–17 | Core/SavedVariables | Preserve overflow recursion behavior. |
...
| `_` / implicit chunk global | external; writes 925, 1433, 6345, 7958 | Receives ignored friend-info/combat-log positional returns, is never read anywhere in the complete source, and only leaks the last discarded value globally. | — | dead | High: complete-file write/read audit. | 02–16 | Core/Init, UI/Summary | Replace with lexical discards after API tuple verification. |
| `online` / implicit chunk global | external; write/read 925, 6345, 7958 | Receives the legacy fifth friend-info return and immediately gates automatic, manual, and login friend publishing. | `friendConnected` | resolved | High: every consumer is a truth gate before whisper publication; Retail `C_FriendList.GetFriendInfoByIndex` now returns `FriendInfo.connected` instead of positional values. | 02–16 | Core/Init, UI/Summary, Core/Events | Make local and read `info.connected`; see `docs/api-baseline.md`. |
...
| `S` / `M` parameter | 953 | Gem contents/color index passed to `CreateFloatingText`, whose color table is `he`; all callers supply the cleared/matched gem color (1023, 2653–2658, 2931–3021). | `gemColor` | resolved | High: full call set and downstream color lookup. | 02–07 | Engine/Scoring, UI/Animations | Hyper uses index 9 white fallback. |
| `f` / `M` parameter | 953 | Boolean result from power-gem creation gates match-4 skill and +25 bonus; straight/intersection match callers supply it (988–990, 2931–2933, 3019–3021). | `createdPowerGem` | resolved | High: full caller provenance and effects. | 02–07 | Engine/Matches, Engine/Scoring | None. |
| `m` / `M` parameter | 953 | True only for hyper/electro destruction paths; selects destruction sound/base bonus before explosion-count bonuses (992–1000, 2658, 2803, 4863). | `destroyedHyperGem` | resolved | High: full call set and branch behavior. | 02–10 | Engine/Matches, Engine/Scoring | None. |
| `a` / `M` parameter | 953 | Explosion count from marked big stars/hyper chains: zero selects HyperDestroy, nonzero ElectroExplode, then adds count-based bonuses (993–1006, 2653–2658, 2803, 2931–3021, 4863). | `explosionCount` | resolved | High: all callers and arithmetic domain. | 02–10 | Engine/Matches, Engine/Scoring | Value `-1` from chained hyper path intentionally avoids positive count bonuses. |
...
| `h` / `M` parameter | 953 | True for T/L intersection scoring; centers the popup on the intersection anchor instead of a straight-run midpoint (1016–1019, 2931, 3019). | `centerOnIntersection` | resolved | High: both callers are cross-arm branches. | 02–07 | Engine/Matches, UI/Animations | None. |
| `u` / `M` parameter | 953 | True only from hyper-cube creation helper `Z`; awards match-5 skill and suppresses the helper's zero-score popup (1020–1028, 2275). | `createdHyperCube` | resolved | High: sole caller and complete effect. | 02–05 | Engine/Matches, UI/Animations | None. |
...
| `unitName` / implicit chunk global | external; write/read 1433–1434 | Receives the legacy environmental-damage destination name and is compared with the player name. | `damagedUnitName` | resolved | High: the adjacent `FALLING` discriminator and player-name comparison establish the destination role. | 03–03 | Core/Init | Make local; modern combat-log extraction requires a compatibility boundary. |
| `damageAmount` / implicit chunk global | external; write/read 1433–1436 | Receives the legacy falling-damage amount and feeds the lethal-damage estimate. | `damageAmount` | resolved | High: it is subtracted from current player health only inside the `ENVIRONMENTAL_DAMAGE`/`FALLING` branch. | 03–03 | Core/Init | Make local; modern combat-log extraction requires a compatibility boundary. |
...
| `bgStatus` / implicit chunk global | external; write/read 1449–1450 | Receives the first `GetBattlefieldStatus` result and is compared with `queued`. | `battlefieldStatus` | resolved | High: current Blizzard Mainline UI source still treats the first return as `status`. | 03–03 | Core/Init | Make local; preserve the exact lowercase `queued` comparison. |
...
| `i`,`o`,`n` / critter callback fields | 1475 | Legacy subevent, source GUID, and destination GUID are tested at 1476. | `subevent`, `sourceGUID`, `destinationGUID` | resolved | High: comparisons against `PARTY_KILL`, player GUID, and target GUID establish all three roles. | 03–03 | Core/Init | Modern extraction must use the verified combat-log compatibility boundary. |
...
| Legacy identifier / lexical scope | Decl. | Observed reads, writes, calls, keys, arithmetic | Working or proposed name | Status | Confidence / evidence | First–last batch | Target subsystem | Open question |
| --- | ---: | --- | --- | --- | --- | --- | --- | --- |
| `t` / ready-check hook parameter | 1506 | Truthiness gates ready-check achievement (1508–1509). | `isReady` | resolved | High: Retail `C_PartyInfo.ConfirmReadyCheck` documents its sole argument as non-nilable Boolean `isReady`; the legacy global is only a deprecation wrapper. | 04–04 | Core/Init | Hook or wrap the current namespaced contract without depending on the legacy global. |
| `skillTrigger` / implicit chunk global | external; write 1509 | Receives one ready-check `CheckSkill` result and is never read anywhere in the complete source. | — | dead | High: complete-file write/read audit. | 04–04 | Core/Init | Discard result without creating a global. |
...
| `t`,`i`,`n` / combat-death fields | 1517 | Legacy damage subevent, destination name, and damage amount drive the event filter and lethal estimate (1518–1523). | `subevent`, `damagedUnitName`, `damageAmount` | resolved | High for semantic roles: `t` is matched against four damage subevents, `i` against the player name, and numeric `n` is subtracted from health. | 04–04 | Core/Init | Do not preserve legacy tuple offsets; extract through a tested Retail compatibility boundary. |
...
| `i`,`n`,`o` / elite-kill fields | 1533 | Subevent, source GUID, destination GUID tested at 1534. | `subevent`, `sourceGUID`, `destinationGUID` | resolved | High: comparisons against `PARTY_KILL`, player GUID, and target GUID establish all three roles. | 04–04 | Core/Init | Modern extraction must use the verified combat-log compatibility boundary. |
...
| `t`,`o`,`i` / rare-kill fields | 1567 | Subevent, source GUID, destination GUID tested at 1568. | `subevent`, `sourceGUID`, `destinationGUID` | resolved | High: comparisons against `PARTY_KILL`, player GUID, and target GUID establish all three roles. | 04–04 | Core/Init | Modern extraction must use the verified combat-log compatibility boundary. |
...
| `teamName1`,`oldTeamRating1`,`newTeamRating1` / implicit globals | external; write 1587 | First arena side's name and rating transition are used at 1589–1593. | `team1Name`, `team1Rating`, `team1RatingNew` | resolved | High: the three values are compared as team identity and an increasing old/new rating pair; current `PVPTeamInfo` preserves the analogous fields. | 04–04 | Core/Init | Replace the obsolete persistent-team membership test with local-player `PVPScoreInfo.ratingChange > 0`. |
| `teamName2`,`oldTeamRating2`,`newTeamRating2` / implicit globals | external; write 1588 | Second arena side's name and rating transition are used at 1589–1597. | `team2Name`, `team2Rating`, `team2RatingNew` | resolved | High: the three values are compared as team identity and an increasing old/new rating pair; current `PVPTeamInfo` preserves the analogous fields. | 04–04 | Core/Init | Replace the obsolete persistent-team membership test with local-player `PVPScoreInfo.ratingChange > 0`. |
...
| `l`,`i`,`o`,`n`,`t`,`a`,`r` / `ScrubLists` locals | 1808 | Mutable row index, entry name, fallback score, selected friend/guild list, classic/timed key, roster-presence flag, and roster name drive duplicate/roster scrubbing through function close (1810–1901). | `row`,`name`,`fallbackScore`,`scoreLists`,`modeKey`,`isPresent`,`rosterName` | resolved | High: complete body and every temporal role are visible. | 04–04 | Core/SavedVariables | Split reused bindings mechanically in rewrite. |
...
| `scoreOffset` / implicit chunk global | external; writes 1819, 1823, 1858, 1861 | Classic/timed fallback decrement (`100`/`.2`) regenerates descending PopCap rows during duplicate and roster scrubs (1843, 1890, 1896). | `fallbackScoreStep` | resolved | High: all writes and arithmetic consumers. | 04–04 | Core/SavedVariables | Must become lexical local without changing fallback sequence. |
...
| `At` / chunk function | 2458 | Creates positioned layered BackdropTemplate frame and is attached as animator method `CreateImage` at 4956. | `createImageFrame` | resolved | High: complete body and final attachment. | 05–10 | UI/GemPool, UI/Animations | None. |
| `a`,`o`,`i`,`n`,`t`,`e`,`l` / `At` parameters | 2458 | Animator method receiver (unused), X, Y, width, height, parent, and overlay flag; colon call sites explain the first argument. | `self`,`x`,`y`,`width`,`height`,`parent`,`overlay` | resolved | High: attachment and all call shapes are complete. | 05–10 | UI/GemPool, UI/Animations | Unused `self` is required by method calling convention. |
| `e` / `At` frame local | 2459 | Shadows parent after initializer; returned frame. | `frame` | resolved | High. | 05–05 | UI/GemPool | None. |
| `n`,`i`,`o`,`t`,`e` / `tt` bindings | 2484–2485 | X, Y, parent, color index, and completed layered gem frame used by the final 8×8 constructor loop. | `x`,`y`,`parent`,`color`,`gem` | resolved | High: complete body and final call site. | 05–17 | UI/GemPool | None. |
...
| --- | ---: | --- | --- | --- | --- | --- | --- | --- |
| `tt` / chunk | 2484 | Completes layered gem-frame construction; final bootstrap calls it 64 times and then nils the factory (2484–2506, 8373–8388). | `createGemFrame` | resolved | High. | 05–17 | UI/GemPool | None. |
...
| `h` / `Qe` local | 2646 | Declared but never read or written in the complete clear helper. | — | dead | High. | 06–06 | Unassigned | Minifier artifact. |
| `o`,`t` / `Qe` locals | 2646 | Created shard object plus direction sign/speed temporary; both are fully consumed by the ten-shard velocity loop. | `shard`,`directionSign` | resolved | High: complete loop data flow. | 06–06 | UI/Animations | None. |
...
| `Ye` / chunk; `t` param | 2749 | Defers or applies mouse-over gem; installed on every final gem frame at 8377. | `handleGemMouseEnter`; `gem` | resolved | High. | 06–17 | UI/GemPool | None. |
...
| `explodeCount` / implicit chunk global | external; writes 2887, 2975; reads 2889–2933, 2977–3021 | Resets per vertical/horizontal match, accumulates `Le` big-star explosion returns, and is passed to scoring `M`. | `explosionCount` | resolved | High: complete accumulation and consumer flow. | 06–07 | Engine/Matches, Engine/Scoring | Must become lexical per-branch local. |
...
| Legacy identifier / scope | Decl. | Evidence | Proposed name | Status | Confidence | First–last | Target | Question |
| --- | ---: | --- | --- | --- | --- | --- | --- | --- |
| `ce` / shadow function; `e` param | 3041 | Mouse-leave handler clears deferred hover or ends eligible effects; installed on every final gem frame at 8378. Shadows earlier half-gem-width constant only for later declarations. | `handleGemMouseLeave`; `gem` | resolved | High. | 07–17 | UI/GemPool | None. |
| `Z` / shadow function; `t` param | 3057 | Main click/swap controller, installed on every final gem frame at 8379. `he` retains the earlier `createHyperGem` binding. | `handleGemClick`; `gem` | resolved | High. | 07–17 | Engine/Input | None. |
...
| `ee` / shadow function; `t` param | 3234 | Drag-release handler maps cursor displacement to a neighboring gem, calls new `Z`, and is installed as every gem's drag-start callback at 8380. `he` retains earlier `createPowerGem`. | `handleGemDragRelease`; `gem` | resolved | High. | 07–17 | Engine/Input | Preserve legacy callback choice. |
...
| `U` / shadow function; `n` param | 3270 | Selects/restyles tab siblings and is attached to Feats tabs (6568, 6585, 6602, 6619) and About tabs (7460, 7477, 7494); shadows color-name table for later code. | `selectTab`; `tabButton` | resolved | High. | 07–15 | UI/Tabs, UI/Skills, UI/About | None. |
...
| `nt` / shadow function; `o`,`r` params | 3288 | Populates leaderboard rows for owner/list key and is attached to tab-3 content as `PopulateScores` at 7167; shadows direction constant for later code. | `populateLeaderboard`; `owner`,`listKey` | resolved | High. | 07–15 | UI/Leaderboard | None. |
...
| `Ze` / shadow function; `t` param | 3619 | Command-text handler parses reset token or toggles window; registered for `/bejeweled` and `/bej` at 8029–8031 after shadowing the earlier dimension constant. | `handleCommand`; `message` | resolved | High: body and registration are complete. | 08–17 | Commands | None. |
...
| `obj` / implicit chunk global | external; read 5701 | No in-file writer exists; absent external injection its Lua value is nil, so popup `OnShow` clears `.parent` before later callers assign a real parent. | `unsetPopupParent` | resolved | High for complete-source behavior; external injection is unsupported evidence. | 12–12 | UI/Popup | Rewrite as explicit nil. |
...
| `D` / shadow function | 5895 | Constructs and installs Flight Path option frame, Start action, and `Ne`-driven timer; invoked by final bootstrap at 8014 after shadowing `tonumber` retained by earlier closures. | `createFlightOptionWindow` | resolved | High. | 12–17 | UI/Flight | None. |
...
| `objectText` / implicit chunk global | external; read 6093 | No in-file writer exists; absent external injection its Lua value is nil and clears the generated slider label before the custom value caption is used. | `unsetSliderLabel` | resolved | High for complete-source behavior; external injection is unsupported evidence. | 13–13 | UI/Flight | Rewrite as explicit nil after UI verification. |
...
| `text` / implicit chunk global | external; write/read 6482–6484 | Brag prompt caption is created, configured, and retained by its frame parent despite leaking its binding globally. | `bragPrompt` | resolved | High: complete object lifecycle. | 13–13 | UI/Summary | Make lexical local. |
...
| `u` / shadow function | 6521 | Constructs/installs four-tab Feats screen, skill award engine/bar/list, statistics, leaderboards, and achievements; invoked and released by final bootstrap (8051, 8393) after shadowing `table.insert` retained by earlier closures. | `createFeatsOfSkillScreen` | resolved | High. | 14–17 | UI/Skills | None. |
...
| `a` / `u` local | 6525 | Feats of Skill screen configured, installed, and populated with four completed tabs/content areas. | `featsOfSkillScreen` | resolved | High. | 14–15 | UI/Skills | None. |
...
| `t` / Feats UI-object local | 6555 | Reassigned across tabs, captions, checkboxes, rank rows, and achievement plates through line 7321. | `uiObject` | resolved | High. | 14–15 | UI/Skills | Split temporal roles in rewrite. |
| `i` / Feats content local | 6621 | Starts as tab-1 skill content, then is reassigned to stats, leaderboard, and achievement content through factory close. | `tabContent` | resolved | High. | 14–15 | UI/Skills | Split four roles in rewrite. |
...
| `o` / Feats panel/content local | 6815 | Reassigned across skill list, stats panels, leaderboard panels, achievement panel, and achievement scroll child through line 7288. | `panelOrContent` | resolved | High. | 14–15 | UI/Skills | Split temporal roles in rewrite. |
| `f` / Feats scroll-frame local | 6822 | Skill-list ScrollFrame, then reassigned to achievement ScrollFrame at 7280; both receive only `BackdropTemplate`. | `scrollFrame` | resolved | High. | 14–15 | UI/Skills | Preserved template defect candidate. |
| `S` / Feats scroll-content local | 6825 | Skill-list content frame, then reassigned to achievement content at 7283. | `scrollContent` | resolved | High. | 14–15 | UI/Skills | None. |
| `n` / Feats layout local | 6831 | Starts nil, later holds vertical offsets 9/11 for tab-2 label/value construction until shadowed by radio callback at 7169. | `verticalOffset` | resolved | High. | 14–15 | UI/Skills | None. |
| `c` / Feats row-chain local | 6832 | Previous skill row, then reset and reused to chain incomplete achievement plates. | `previousRow` | resolved | High. | 14–15 | UI/Skills | Split roles in rewrite. |
| `r`,`p` / row-hover functions; `e` params | 6833, 6836 | OnEnter/OnLeave callbacks color skill row text white or gold; `r` is captured before a font-size local shadows it at 7050. | `highlightRow`,`unhighlightRow`; `row` | resolved | High. | 14–15 | UI/Skills | None. |
...
| `h` / skill-text-size local | 6884 | Constant 13 used for skill headers/items, then reassigned for achievement plate text at 7289. | `skillTextSize` | resolved | High. | 14–15 | UI/Skills | None. |
...
| `n` / tab-1 `UpdateSkillScreen` param | 6921 | Skill content receives title/row/header/scroll updates and first-show tutorial; callback closes at 7002. | `skillContent` | resolved | High. | 14–15 | UI/Skills | None. |
...
| second `t` / refresh declaration at 6931 | 6931 | Section index in completed population loop. | `sectionIndex` | resolved | High. | 14–15 | UI/Skills | None. |
| `l`,`o`,`h`,`s` / refresh locals | 6930–6931 | Eligibility/color tier, row, section-open state, and last populated row through callback close. | `tier`,`row`,`sectionOpen`,`lastRow` | resolved | High. | 14–15 | UI/Skills | None. |
| `a`,`d`,`r` / refresh locals | 6932–6934 | Skill-data table, row text height, and final scroll height. | `skillData`,`rowHeight`,`scrollHeight` | resolved | High. | 14–15 | UI/Skills | None. |
...
| `t`,`i` / refresh population loops | 6945, 6954 | Section index and reverse skill-data entry index through completed refresh. | `sectionIndex`,`skillDataIndex` | resolved | High. | 14–15 | UI/Skills | None. |
| `foundFeat` / implicit chunk global | external; write/read 6953–6986 | Undeclared per-section boolean tracks whether any eligible feat was assigned; no later read through line 7500. | `foundFeat` | resolved | High for role; accidental global. | 14–15 | UI/Skills | Must become callback-local. |
| `itemID` / implicit chunk global | external; write/read 6971–6973 | Undeclared row-slot index copied from header `.current`, incremented, and used for item lookup; no later read through line 7500. | `itemId` | resolved | High for role; accidental global. | 14–15 | UI/Skills | Must become callback-local. |
...
## Batch 15 declarations and scopes

| Legacy identifier / scope | Decl. | Evidence | Proposed name | Status | Confidence | First–last | Target | Question |
| --- | ---: | --- | --- | --- | --- | --- | --- | --- | --- |
| `t` / stats-tab `OnShow` param | 7009 | Tab-2 content receives all refreshed best/stat value writes before loop-local shadow. | `statsContent` | resolved | High. | 15–15 | UI/Skills, UI/Stats | None. |
| `e` / favorite-color local | 7024 | Index of maximum `stats.gemMatch` count, initialized to 1 and updated by scan. | `favoriteColorIndex` | resolved | High. | 15–15 | UI/Stats | Tie handling checks only indices 1 and 2. |
| `n` / stats-tab callback local | 7025 | Nil declaration is never assigned/read before callback close. | — | dead | High. | 15–15 | UI/Stats | Minifier artifact. |
| `t` / favorite-color loop | 7026 | Gem-color index 2 through end; shadows callback parameter only inside loop. | `colorIndex` | resolved | High. | 15–15 | UI/Stats | None. |
| `r` / stats-font local | 7050 | Constant 16 passed to Personal Bests and Fun Statistics captions; shadows captured hover function after attachment. | `statsFontSize` | resolved | High. | 15–15 | UI/Stats | None. |
| `e` / leaderboard-tab `OnShow` param | 7160 | Tab-3 content supplies ID and `PopulateScores` receiver. | `leaderboardContent` | resolved | High. | 15–15 | UI/Leaderboard | None. |
| `t` / leaderboard-tab mode local | 7161 | Content ID selects friends (3) or guild (4). | `tabId` | resolved | High. | 15–15 | UI/Leaderboard | None. |
| `n` / leaderboard-radio function; `e` param | 7169 | Radio callback clears both checks, selects clicked checkbox, copies ID to parent, and hide/shows parent for refresh. | `selectLeaderboardScope`; `checkbox` | resolved | High. | 15–15 | UI/Leaderboard | None. |
| first `t` / leaderboard-radio locals | 7170 | Extracted checkbox-name suffix is never read and is shadowed at line 7174. | — | shadowed | High. | 15–15 | UI/Leaderboard | Minifier/removed setting logic. |
| second `t` / leaderboard-radio local | 7174 | Checkbox parent whose ID/visibility trigger score refresh. | `leaderboardContent` | resolved | High. | 15–15 | UI/Leaderboard | None. |
| `n` / leaderboard row loops | 7205, 7234 | Distinct classic and timed rank indices 1–10. | `rankIndex` | resolved | High. | 15–15 | UI/Leaderboard | None. |
| `e` / overwritten tab-4 `OnShow` param | 7258 | Achievement content receiver in copied leaderboard callback; script is replaced at 7410 before normal use. | `achievementContent` | resolved | High; behavior overwritten. | 15–15 | UI/Skills | Preserve construction order. |
| `t` / overwritten tab-4 mode local | 7259 | Frame ID would select guild population in the callback replaced at 7410. | `tabId` | resolved | High; behavior overwritten. | 15–15 | UI/Skills | None. |
| `n`,`a` / achievement preallocation loops | 7292–7293 | Skill-data type 6–7 and per-type record index used to build plate rows. | `skillType`,`achievementIndex` | resolved | High. | 15–15 | UI/Skills | None. |
| `r` / achievement `UpdateSkillScreen` param | 7324 | Tab-4 content receives title/plate/height updates. | `achievementContent` | resolved | High. | 15–15 | UI/Skills | None. |
| `t` / achievement rank local | 7325 | Rank snapshot is never read and is shadowed at line 7327. | — | shadowed | High. | 15–15 | UI/Skills | Minifier artifact. |
| `f` / achievement refresh local | 7326 | Current skill points gate unlocked entries; shadows outer scroll-frame binding inside callback. | `skillPoints` | resolved | High. | 15–15 | UI/Skills | None. |
| `t` / achievement locals at 7327–7328 | 7327–7328 | Nil then numeric-one bindings are successively shadowed by line-7329 `t` without reads. | — | shadowed | High. | 15–15 | UI/Skills | Minifier artifacts. |
| four `n` / achievement declaration | 7329 | First four same-statement `n` bindings are hidden by later duplicates. | — | shadowed | High. | 15–15 | UI/Skills | None. |
| final `n` / achievement declaration | 7329 | Surviving binding is immediately shadowed by line-7330 `n` without a read. | — | shadowed | High. | 15–15 | UI/Skills | None. |
| `t` / achievement row local | 7329 | Current achievement plate during reset/population loops. | `achievementRow` | resolved | High. | 15–15 | UI/Skills | None. |
| `n` / achievement data local | 7330 | Complete skill-data table indexed by types 6–7; shadows final line-7329 binding. | `skillData` | resolved | High. | 15–15 | UI/Skills | None. |
| first `o` / achievement locals | 7331 | Skill-text-size value is never read and is shadowed at 7333. | — | shadowed | High. | 15–15 | UI/Skills | Minifier artifact. |
| `d` / achievement refresh local | 7332 | Computed scroll height, initialized 1 and incremented 51 per unlocked row. | `scrollHeight` | resolved | High. | 15–15 | UI/Skills | None. |
| second `o` / achievement local | 7333 | Nil binding is hidden by loop-local `o` during all uses and never read afterward. | — | dead | High. | 15–15 | UI/Skills | Minifier artifact. |
| `l`,`c`,`a` / achievement chain locals | 7334 | Last completed row, last incomplete row, and first incomplete row used to form two ordered groups. | `completedTail`,`incompleteTail`,`firstIncomplete` | resolved | High. | 15–15 | UI/Skills | None. |
...
| `i` / About local | 7417 | About screen configured/installed and populated with three completed tabs/content areas. | `aboutScreen` | resolved | High. | 15–16 | UI/About | None. |
| `r` / About backdrop local | 7423 | Backdrop descriptor populated and applied to About screen. | `backdropInfo` | resolved | High. | 15–15 | UI/About | None. |
| `t` / About `OnShow` param | 7432 | About screen shows tabs and coordinates pause/game/summary/hint alpha. | `aboutScreen` | resolved | High. | 15–15 | UI/About | None. |
| `t` / About `OnHide` param | 7441 | Callback parameter is never read. | — | dead | High. | 15–15 | UI/About | None. |
| `t` / About UI-object local | 7447 | Reassigned across three tabs, tutorial textures/captions, story/version text, and credits through factory close. | `uiObject` | resolved | High. | 15–16 | UI/About | Split temporal roles in rewrite. |
| `a` / About content local | 7496 | Reassigned across Tutorial, PopCap story, and Credits content frames stored as tab1–tab3 content. | `aboutContent` | resolved | High. | 15–16 | UI/About | Split three roles in rewrite. |
| `n` / About child local | 7500 | Reassigned across tutorial panels and story/credits inner panels through factory close. | `contentPanel` | resolved | High. | 15–16 | UI/About | Split roles in rewrite. |

## Batch 15 resolution policy

Feats factory `u` and both refresh callbacks close, resolving stats, leaderboard, achievement ordering/counts, and all previously open refresh locals. The overwritten tab-4 callback is recorded as construction-order evidence. About factory `m`, About screen/content, and boundary child `n` remain working until batch 16.

## Batch 16 declarations and scopes

| Legacy identifier / scope | Decl. | Evidence | Proposed name | Status | Confidence | First–last | Target | Question |
| --- | ---: | --- | --- | --- | --- | --- | --- | --- | --- |
| `o` / About art-metadata local | 7507 | `largeText` metadata reassigned from Tutorial 1 through Tutorial 3 and PopCap art. | `artMetadata` | resolved | High. | 16–16 | UI/About | Tutorial 2 supplies eight UV values. |
| `i` / Options local | 7634 | Main window receives alpha, keyboard scripts, keybind button, and override bindings. | `window` | resolved | High. | 16–16 | UI/Options | None. |
| `t` / Options-screen local | 7640 | Options frame configured/installed before line-7716 offset local shadows it. | `optionsScreen` | resolved | High. | 16–16 | UI/Options | None. |
| first `a` / Options backdrop local | 7644 | Backdrop descriptor applied to Options screen/content panel before line-7677 shadow. | `backdropInfo` | resolved | High. | 16–16 | UI/Options | None. |
| `t` / Options show/hide callback params | 7652, 7658 | Distinct frame parameters are never read; callbacks pause/restore alpha through globals. | — | dead | High. | 16–16 | UI/Options | None. |
| `n` / Options control local | 7665 | Reassigned from title/caption through created checkbox controls to line 7783. | `control` | resolved | High. | 16–16 | UI/Options | Split roles in rewrite. |
| `o` / Options-content local | 7669 | Inner panel parents every setting control. | `optionsPanel` | resolved | High. | 16–16 | UI/Options | None. |
| second `a` / Options offset local | 7677 | Numeric vertical origin 5; shadows consumed backdrop descriptor and positions keybind/sliders. | `topOffset` | resolved | High. | 16–16 | UI/Options | None. |
| first `l` / Options keybind local | 7682 | Keybind button configured, scripted, and stored on main window before sound-handler shadow. | `keybindButton` | resolved | High. | 16–16 | UI/Options | None. |
| `t` / keybind-button callback param | 7686 | Button toggles capture state using `savedText`, highlight, and label. | `keybindButton` | resolved | High. | 16–16 | UI/Options | None. |
| `t` / game-alpha slider callback param | 7707 | Slider supplies live alpha value after fade descriptors update. | `slider` | resolved | High. | 16–16 | UI/Options | None. |
| `t` / Options vertical-offset local | 7716 | Starts at 100 and increments through sound/general/auto-open settings; shadows Options frame binding after storage. | `verticalOffset` | resolved | High. | 16–16 | UI/Options | None. |
| second `l` / sound-radio function; `e` param | 7718 | Callback clears all sound fields, extracts clicked setting key, sets it to 1, and checks control. | `selectSoundMode`; `checkbox` | resolved | High. | 16–16 | UI/Options, UI/Sound | None. |
| `t` / sound-setting local | 7719 | Setting suffix extracted from checkbox global name. | `settingKey` | resolved | High. | 16–16 | UI/Options, UI/Sound | None. |
| `t` / hide-minimap callback param | 7736 | Checkbox name/value persist setting and toggle minimap visibility. | `checkbox` | resolved | High. | 16–16 | UI/Options, UI/Minimap | None. |
| `t` / disable-hints callback param | 7767 | Checkbox persists setting and chooses hint creation versus shutdown. | `checkbox` | resolved | High. | 16–16 | UI/Options, UI/Animator | None. |
| `e` / disable-hints local | 7772 | Existing hint object receives inactive effect, hidden alpha/state reset. | `hintObject` | resolved | High. | 16–16 | UI/Animator | None. |
| `e`,`t` / key-up callback params | 7798 | Main window and released key; modifier-key releases trigger modifier recomputation. | `window`,`key` | resolved | High. | 16–16 | UI/Options | None. |
| `t` / key-up separator local | 7800 | Empty/`-` separator used while rebuilding ALT-CTRL-SHIFT string; shadows consumed key parameter. | `separator` | resolved | High. | 16–16 | UI/Options | None. |
...
| `t` / flight-end mode local | 7894 | Boolean `(n.gameMode == k)` is computed but never read. | — | dead | High. | 16–16 | Core/Events, UI/Flight | Minifier artifact. |
| `a`,`l` / login stats locals | 7941–7942 | Classic and timed profile stats tables. | `classicStats`,`timedStats` | resolved | High. | 16–16 | Core/Events, Persistence/Stats | None. |
| `i`,`o` / login score locals | 7943–7944 | Start as numeric classic/timed scores, are overwritten before read by encoded fields `[v]`, then used in publish payload. | `classicEncodedScore`,`timedEncodedScore` | resolved | High. | 16–16 | Core/Events, Network/Publishing | Preserve overwrite behavior. |
| first `t` / login local | 7945 | Player name used to build rank-name prefix and hash initializer. | `playerName` | resolved | High. | 16–16 | Core/Events | None. |
| first `n` / login local | 7946 | Rank plus player-name prefix ending `*`, duplicated in final payload. | `rankNamePrefix` | resolved | High. | 16–16 | Network/Publishing | Duplication is wire behavior. |
| second `t` / login local | 7947 | Player-name hash/checksum result is never read before shadow at line 7956. | — | dead | High. | 16–16 | Core/Events | Minifier/removed checksum seed. |
| second `n` / login local | 7950 | Final `rankName*classic*rankName*timed` high-score payload. | `publishPayload` | resolved | High. | 16–16 | Network/Publishing | Preserve exact separators/prefix duplication. |
...
| `o` / login friend loop | 7957 | Iterates friend indices and shadows pre-loop binding. | `friendIndex` | resolved | High. | 16–16 | Network/Publishing | None. |
...
| `t`,`e` / board-tile loops | 8060–8061 | Row/column values 0–3 place sixteen 100×100 repeated texture crops at `(column×100, -row×100)`. | `tileRow`,`tileColumn` | resolved | High. | 17–17 | UI/GemPool | None. |
...
| `t` / status `SetText` metadata local | 8106 | Selected `largeText` array drives dimensions, rectangular/rotated UVs, and RGB tint. | `textMetadata` | resolved | High. | 17–17 | UI/HUD | Preserve length-based 9-versus-13 entry shape. |
...
| `t` / splash-hide param | 8322 | Splash callback parameter is unused while account totals/profile fields drive achievement check. | — | dead | High. | 17–17 | UI/HUD, Engine/Skills | None. |
