...
| `_` / implicit chunk global | external; writes 925, 1433, 6345, 7958 | Receives repeated ignored friend-info returns and a combat-log positional return; each assignment survives globally, and no read appears through line 8000. | discarded positional return | unresolved | High for accidental global writes. | 02–16 | Core/Init, UI/Summary | Must become local discards after API verification. |
| `online` / implicit chunk global | external; write/read 925, 6345, 7958 | Receives the fifth friend-info return and immediately gates automatic, manual, and login friend publishing. | `friendOnline` | working | High for intended role; API return contract pending. | 02–16 | Core/Init, UI/Summary, Core/Events | Must become local and use current API shape. |
