...
9. `Engine/Input.lua` — authoritative selection and move-session coordination, optimistic adjacent swaps, match validation, immediate engine rollback for invalid moves, legacy hyper activation, move accounting, cascade/scoring handoff, and presentation locking. Pause/resume, levels, restore, and game-over remain follow-on session work.
